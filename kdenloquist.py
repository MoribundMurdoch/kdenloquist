...
class KdenLoquist:
    ...
    def _build_panel_contents(self, p):
        ...
        self._mask_lbl = tk.Label(b2, text="No points placed", bg=PANEL_BG, fg=FG_DIM, font=("Segoe UI", 8, "italic"))
        ...
        tk.Spinbox(of_row, from_=0, to=999, textvariable=self.v_offset, width=5, bg=BTN_BG, fg=FG, buttonbackground=PANEL_BG, highlightthickness=0).pack(side=tk.RIGHT)

        clr_row = tk.Frame(b3, bg=PANEL_BG); clr_row.pack(fill=tk.X, pady=2)
        tk.Label(clr_row, text="Inner Mouth Colour", bg=PANEL_BG, fg=FG_DIM, font=("Segoe UI", 8)).pack(side=tk.LEFT)
        self._color_btn = tk.Button(clr_row, bg=self.v_mouth_color.get(), width=3, relief=tk.FLAT, cursor="hand2", command=self._pick_color)
        ...
        tk.Label(fps_row, text="Frame Rate (FPS)", bg=PANEL_BG, fg=FG_DIM, font=("Segoe UI", 8)).pack(side=tk.LEFT)
        ttk.Combobox(fps_row, textvariable=self.v_fps, width=5, values=[12, 23, 24, 25, 29, 30, 48, 50, 60], state="readonly").pack(side=tk.RIGHT)
        LabeledSlider(b4, "Export Quality (CRF)", self.v_crf, 0, 51, fmt="{:.0f}  (lower=better)").pack(fill=tk.X, pady=2)
        ...
        self._prog_lbl = tk.Label(b5, text="Ready.", bg=PANEL_BG, fg=FG_DIM, font=("Segoe UI", 8))
    ...
    def _on_click(self, e):
        if self.image is None or self.mask_closed: return
    ...
    def _on_double_click(self, e):
        ...
        if len(self.mask_points) > 0: self.mask_points.pop()
        self._close_mask()
    ...
    def _draw_polygon_overlay(self):
        ...
        for i in range(len(cpts) - 1):
            self._canvas.create_line(*cpts[i], *cpts[i + 1], fill=ACCENT, width=2)
    ...
    def _start_export(self):
        ...
        if not self.mask_closed:
            if not messagebox.askyesno("Mask not closed", "The mask isn't closed yet. Export anyway?"): return
