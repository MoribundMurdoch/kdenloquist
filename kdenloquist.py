...
============================================================
  ...
  Workflow:
    ...
    2. Click points around the mouth to build a polygon mask
       - Right-click or press Z to undo the last point
       - Double-click OR click near the first point to close
...
def render_frame_ellipse(base_rgba: np.ndarray, points: list,
                         amplitude: float, softness: float,
                         inner_color: tuple) -> np.ndarray:
    """
    Ellipse-grow mode: scales the polygon shape from its centroid.
    """
    result = base_rgba.copy()
    if not points or len(points) < 3 or amplitude <= 0.001:
        return result
    ...
    scaled = (cx + (pts_np - [cx, cy]) * amplitude).astype(np.int32)
    mask   = np.zeros((h_img, w_img), dtype=np.float32)
    cv2.fillPoly(mask, [scaled], 1.0)
    ...
    m4 = mask[:, :, np.newaxis]
    result = (base_rgba.astype(np.float32) * (1.0 - m4) +
              inner_arr.astype(np.float32) * m4).clip(0, 255).astype(np.uint8)
    result[:, :, 3] = base_rgba[:, :, 3]
    return result
...
class KdenLoquist:
    ...
    def _build_panel_contents(self, p):
        ...
        # ── Animation ──────────────────────────────────────
        s3 = Section(p, "🎬  Animation"); s3.pack(**pad); b3 = s3.body
        ...
        LabeledSlider(self._puppet_frame, "Jaw Drop Amount", self.v_jaw_drop, 0.1, 1.0).pack(fill=tk.X, pady=2)
        LabeledSlider(self._puppet_frame, "Hinge Position",  self.v_hinge,   0.1, 0.9).pack(fill=tk.X, pady=2)
        tk.Label(self._puppet_frame,
                 text="Hinge: 0=top of mask, 1=bottom\n(~0.4 works for most faces)",
        ...
        tk.Label(s6.body,
                 text=("MASK DRAWING:\n"
                       ...
                       "  Jaw Drop → how wide it opens\n\n"
                       "ELLIPSE mode:\n"
                       "  Polygon shape scales from centroid."),
                 bg=PANEL_BG, fg=FG_DIM, font=("Segoe UI", 8),
    ...
    def _draw_polygon_overlay(self):
        ...
        # Hinge line (puppet mode only, when closed)
        if self.mask_closed and self.v_mode.get() == "puppet" and len(pts) >= 3:
            pts_np = np.array(pts)
            x1, y1 = pts_np.min(axis=0)
            x2, y2 = pts_np.max(axis=0)
            mh     = max(y2 - y1, 1)
        ...
        if not self.mask_closed:
            cx0, cy0 = cpts[0]
            self._canvas.create_text(cx0 + POINT_R + 30, cy0,
